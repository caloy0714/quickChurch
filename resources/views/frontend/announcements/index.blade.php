        ...
        <div class="col-md-12">
            @can('announcement_create')
                <div style="margin-bottom: 10px;" class="row">
                    <div class="col-lg-12">
                        <a class="btn btn-success" href="{{ route('frontend.announcements.create') }}">
                                    ...
                                    <tr data-entry-id="{{ $announcement->id }}">
                                        ...
                                        <td>
                                            @can('announcement_show')
                                                <a class="btn btn-xs btn-primary" href="{{ route('frontend.announcements.show', $announcement->id) }}">
                                                    {{ trans('global.view') }}
                                                </a>
                                            @endcan

                                            @can('announcement_edit')
                                                <a class="btn btn-xs btn-info" href="{{ route('frontend.announcements.edit', $announcement->id) }}">
                                                    {{ trans('global.edit') }}
                                                </a>
                                            @endcan

                                            @can('announcement_delete')
                                                <form action="{{ route('frontend.announcements.destroy', $announcement->id) }}" method="POST" onsubmit="return confirm('{{ trans('global.areYouSure') }}');" style="display: inline-block;">
...
@can('announcement_delete')
  ...
  let deleteButton = {
    ...
    url: "{{ route('frontend.announcements.massDestroy') }}",
